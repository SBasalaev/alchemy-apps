...
if(ev.value==exit){ndone=true;e.value=exit}
...
else if(ev.value==bck){fom.clear();SetScreen(prvscr)}
}catch{SetScreen(lis)}
}
...
okk=true
fom.clear()
if(lislen>7){if(gtn<7){for(var i=0,i<7,i+=1){lis.set_index(i)}}}
for(var i=0,i<gtn,i+=1){lis.set_index(i)}
fom.clear()
SetScreen(prvscr)
}
...
if(x==lislen&&!ckiv)
{
for(var i=0,i<7,i+=1){lis.set_index(i)}
x=0
}
fn=lis.get_string(x)
Load_song()
...
scgra.draw_string(" *",tempwidth,h*9/2)
shrtcts.refresh()
ui_set_screen(shrtcts)
tempwidth+=10
sleep(1)
...
scgra.fill_rect(0,0,w*10,h)
scgra.set_color(0xffffff)
scgra.draw_string("...About...",tempwidth,0)
shrtcts.refresh()
ic=tempwid
do
{
...
scgra.set_color(0xffffff)
scgra.draw_string("Music Player for Alchemy OS.",ic,h)
ic-=5
shrtcts.refresh()
sleep(1)
}while(tempevent==0&&ic>=0)
scgra.set_color(0x000000)
scgra.fill_rect(0,h,w*10,h)
scgra.set_color(0xffffff)
...
scgra.set_color(0xffffff)
scgra.draw_string("Created by : Swapnil.",ic,h*2)
ic-=5
shrtcts.refresh()
sleep(1)
}while(tempevent==0&&ic>=0)
...
scgra.draw_string("email : jack_5@asia.com",ic,h*3)
ic-=5
shrtcts.refresh()
sleep(1)
}while(tempevent==0&&ic>=0)
